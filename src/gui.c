...
SDL_FColor getHalfLifeCol(const double halflifeSeconds, const uint8_t valueType, const uint8_t darkTheme){
  ...
  if((halflifeSeconds >= 1.0E40)||((halflifeSeconds > 1.0E15) && ((valueType == VALUETYPE_GREATERTHAN) || (valueType == VALUETYPE_GREATEROREQUALTHAN)))){
    ...
    if(darkTheme){
      col.r = 1.0f; col.g = 0.8f + 0.15f*colChangeFactor; col.b = 0.8f + 0.15f*colChangeFactor;
    }else{
    ...
    if(darkTheme){
      col.r = 0.925f + 0.075f*colChangeFactor; col.g = 0.6f + 0.2f*colChangeFactor; col.b = 0.4f + 0.4f*colChangeFactor;
    }else{
    ...
    if(darkTheme){
      col.r = 0.55f + 0.375f*colChangeFactor; col.g = 0.8f - 0.2f*colChangeFactor; col.b = 0.375f + 0.025f*colChangeFactor;
    }else{
    ...
    if(darkTheme){
      col.r = 0.2f + 0.35f*colChangeFactor; col.g = 0.8f; col.b = 0.55f - 0.175f*colChangeFactor;
    }else{
      col.r = 0.3f - 0.075f*colChangeFactor; col.g = 0.6f - 0.175f*colChangeFactor; col.b = 0.75f + 0.05f*colChangeFactor;
    }
  }else if(halflifeSeconds > 1.0){ //text color inversion point for dark theme
    float colChangeFactor = (float)(SDL_log10(halflifeSeconds - 1.0)/SDL_log10(5.0E2 - 1.0)); //value from 0 to 1
    if(darkTheme){
      col.r = 0.3f - 0.1f*colChangeFactor; col.g = 0.625f + 0.175f*colChangeFactor; col.b = 0.70f - 0.15f*colChangeFactor;
    }else{
      col.r = 0.5f - 0.2f*colChangeFactor; col.g = 1.0f - 0.4f*colChangeFactor; col.b = 0.7f + 0.05f*colChangeFactor;
    }
    ...
    //SDL_Log("%f - %f %f %f\n",(double)colChangeFactor,(double)col.r,(double)col.g,(double)col.b);
    ...
    if(darkTheme){
      col.r = 0.225f + 0.075f*colChangeFactor; col.g = 0.375f + 0.15f*colChangeFactor; col.b = 0.525f - 0.1f*colChangeFactor;
    }else{
...
SDL_FColor get2PlusCol(const double e2PlusKeV, const double halflifeSeconds, const uint8_t darkTheme){
  ...
  if(e2PlusKeV >= 4000.0){
    ...
    col.g = 0.0f;
    col.b = 0.8f;
  }else if(e2PlusKeV >= 2000.0){ //box color inversion point
    col.r = 0.0f;
    col.g = 0.0f;
    col.b = 1.0f;
    ...
    col.g = 0.65f;
    col.b = 0.0f;
  }else if(e2PlusKeV >= 150.0){
    col.r = 1.0f;
    col.g = 0.5f;
    col.b = 0.0f;
  }else if(e2PlusKeV >= 125.0){
    col.r = 1.0f;
    col.g = 0.35f;
    col.b = 0.0f;
...
SDL_FColor getSnpCol(const double snpKeV, const uint8_t darkTheme){
  SDL_FColor col;
  col.a = 1.0f;
  if(snpKeV == 0.0){
    if(darkTheme){
...
SDL_FColor getQaCol(const double qaKeV, const uint8_t darkTheme){
  SDL_FColor col;
  col.a = 1.0f;
  if(qaKeV == 0.0){
    if(darkTheme){
...
SDL_FColor getQbCol(const double qbKeV, const uint8_t darkTheme){
  SDL_FColor col;
  col.a = 1.0f;
  if(qbKeV == 0.0){
    if(darkTheme){
...
SDL_FColor getNumLvlsCol(const uint16_t numLvls, const double halflifeSeconds, const uint8_t darkTheme){
  ...
  if(darkTheme){
    ...
    if(col.g >= 0.8f){ col.g -= 0.1f; }
    if(col.b >= 0.8f){ col.b -= 0.1f; }
  }
  return col;
}
...
#define LOWBOX_TXT_CHANGE_ZOOM 30.0f
...
void drawIsomerSpinBoxLabel(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const float xPos, const float yPos, const float boxWidth, const float boxHeight, SDL_Color col, const uint16_t nuclInd, const uint32_t isomerLvl, const uint8_t isomerMVal){
  ...
  if(boxHeight > 42.0f){
    uint8_t fontSizeInd = FONTSIZE_LARGE;
    uint8_t smallerFontSizeInd = FONTSIZE_NORMAL;
    if(state->ds.chartZoomScale >= LOWBOX_TXT_CHANGE_ZOOM){
      fontSizeInd = FONTSIZE_HUGE;
      smallerFontSizeInd = FONTSIZE_LARGE;
    }
    drawXPos = xPos + labelMargin;
    const float totalLblHeight = SDL_roundf(getTextHeight(rdat,fontSizeInd,getElemStr((uint8_t)Z))/rdat->uiDPIScale);
    ...
    drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,fontSizeInd,255,tmpStr,ALIGN_LEFT,16384); //draw element label
    ...
    if(dat->ndat.levels[isomerLvl].numSpinParVals > 0){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,jPiStr,ALIGN_RIGHT,16384); //draw spin-parity label
    }else{
      drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_RIGHT,16384); //draw unknown label
    }
  }
}
...
void drawIsomerDecayModeBoxLabel(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const float xPos, const float yPos, const float boxWidth, const float boxHeight, SDL_Color col, const uint16_t nuclInd, const uint32_t isomerLvl, const uint8_t isomerMVal, const uint8_t dcyMode){
  ...
  if(boxHeight > 42.0f){
    ...
    if(dat->ndat.levels[isomerLvl].numSpinParVals > 0){
      ...
      getSpinParStr(tmpStr,&dat->ndat,isomerLvl);
      if(state->ds.chartZoomScale >= LOWBOX_TXT_CHANGE_ZOOM){
        drawTextAlignedSized(rdat,drawXPos,drawYPos-(4.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw spin-parity label
      }else{
        drawTextAlignedSized(rdat,drawXPos,drawYPos-(2.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw spin-parity label
      }
      ...
      if(drawSpace > getTextWidth(rdat,fontSizeInd,tmpStr)/rdat->uiDPIScale){
        if(state->ds.chartZoomScale >= LOWBOX_TXT_CHANGE_ZOOM){
          drawTextAlignedSized(rdat,drawXPos,drawYPos+(24.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw decay mode label
        }else{
          drawTextAlignedSized(rdat,drawXPos,drawYPos+(18.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw decay mode label
        }
      }else{
        if(state->ds.chartZoomScale >= LOWBOX_TXT_CHANGE_ZOOM){
          drawTextAlignedSized(rdat,drawXPos,drawYPos+(24.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,"(...)",ALIGN_RIGHT,16384);
        }else{
          drawTextAlignedSized(rdat,drawXPos,drawYPos+(18.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,"(...)",ALIGN_RIGHT,16384);
        }
      }
    }else{
      ...
      if(drawSpace > getTextWidth(rdat,fontSizeInd,tmpStr)/rdat->uiDPIScale){
        drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw decay mode label
      }else{
        drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,"(...)",ALIGN_RIGHT,16384);
      }
    }
  }
}
  ...
  if(boxHeight > 42.0f){
    ...
    if(dat->ndat.levels[isomerLvl].numSpinParVals > 0){
      ...
      if(state->ds.chartZoomScale >= LOWBOX_TXT_CHANGE_ZOOM){
        ...
        drawTextAlignedSized(rdat,drawXPos,drawYPos+(24.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw half-life label
      }else{
        getSpinParStr(tmpStr,&dat->ndat,isomerLvl);
        drawTextAlignedSized(rdat,drawXPos,drawYPos-(2.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw spin-parity label
        getHalfLifeStr(tmpStr,dat,isomerLvl,1,0,state->ds.useLifetimes);
        drawTextAlignedSized(rdat,drawXPos,drawYPos+(18.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw half-life label
      }
    }else{
      //only half-life known
      getHalfLifeStr(tmpStr,dat,isomerLvl,1,0,state->ds.useLifetimes);
      drawTextAlignedSized(rdat,drawXPos,drawYPos+(10.0f*state->ds.uiUserScale),col,smallerFontSizeInd,255,tmpStr,ALIGN_RIGHT,16384); //draw half-life label
    }
  }
}
...
void drawNuclBoxLabelDetails(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const float xPos, const float yPos, const float boxWidth, const float boxHeight, SDL_Color col, const uint16_t nuclInd){
  ...
  yOffsetLimit += (uint8_t)(floorf((boxHeight - 90.0f*state->ds.uiUserScale)/(30.0f*state->ds.uiUserScale)));
  ...
  if(state->chartView == CHARTVIEW_HALFLIFE){
    ...
    if(getTextWidth(rdat,labelFontInd,tmpStr) > (maxLblWidth*rdat->uiDPIScale)){
      drawYOffsets = 2;
    }
    if((yOffsets+drawYOffsets) <= yOffsetLimit){
      ...
      if(dat->ndat.nuclData[nuclInd].abundance.unit == VALUE_UNIT_PERCENT){
        ...
        if(((yOffsets+drawYOffsets) <= yOffsetLimit)||((drawYOffsets == 1)&&(dat->ndat.levels[gsLevInd].numDecModes==0))){
          ...
          drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw abundance label
          yOffsets += drawYOffsets;
          for(int8_t i=0; i<dat->ndat.levels[gsLevInd].numDecModes; i++){
            ...
            if(((yOffsets+drawYOffsets) <= yOffsetLimit)||((drawYOffsets == 1)&&(i==(dat->ndat.levels[gsLevInd].numDecModes-1)))){
              drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw decay mode label
              //SDL_Log("height: %f\n",(double)height);
              yOffsets += drawYOffsets;
            }else{
    ...
    if(dat->ndat.nuclData[nuclInd].abundance.unit == VALUE_UNIT_PERCENT){
      ...
      if(((yOffsets+drawYOffsets) <= yOffsetLimit)||((drawYOffsets == 1)&&(dat->ndat.levels[gsLevInd].numDecModes==0))){
        ...
        for(int8_t i=0; i<dat->ndat.levels[gsLevInd].numDecModes; i++){
          ...
          //SDL_Log("%s\n",tmpStr);
          drawYOffsets = 1;
          if(getTextWidth(rdat,labelFontInd,tmpStr) > (1.3f*maxLblWidth*rdat->uiDPIScale)){
            drawYOffsets = 3;
          }else if(getTextWidth(rdat,labelFontInd,tmpStr) > (maxLblWidth*rdat->uiDPIScale)){
            drawYOffsets = 2;
          }
          if(((yOffsets+drawYOffsets) <= yOffsetLimit)||((drawYOffsets == 1)&&(i==(dat->ndat.levels[gsLevInd].numDecModes-1)))){
            drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw decay mode label
    ...
    if(plus2Lvl != MAXNUMLVLS){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw 2+ energy label
    }
  }else if(state->chartView == CHARTVIEW_R42){
    double r42 = getR42(&dat->ndat,nuclInd);
    if(r42 > 0.0){
      SDL_snprintf(tmpStr,32,"R₄₂ = %0.2f",r42);
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw R42 label
    }
    ...
    if(getMostProbableSpin(&dat->ndat,gsLevInd) >= 255.0){
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_CENTER,maxLblWidth); //draw unknown label
    }else{
      char jPiStr[64];
      getSpinParStr(tmpStr,&dat->ndat,gsLevInd);
      SDL_snprintf(jPiStr,64,"%s: %s",dat->strings[dat->locStringIDs[LOCSTR_JPI]],tmpStr);
      drawYOffsets = 1;
      if(getTextWidth(rdat,labelFontInd,jPiStr) > (maxLblWidth*rdat->uiDPIScale)){
        drawYOffsets = 2;
      }
      if((yOffsets+drawYOffsets) <= yOffsetLimit){
        drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,jPiStr,ALIGN_CENTER,maxLblWidth); //draw spin-parity label
        //SDL_Log("height: %f\n",(double)height);
        yOffsets += drawYOffsets;
      }else{
        drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,"(...)",ALIGN_CENTER,maxLblWidth);
      }
    }
    ...
    if(beA > 0.0){
      ...
      SDL_strlcat(tmpStr," keV",32);
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw BE/A label
    }
    ...
    if(dat->ndat.nuclData[nuclInd].sn.val != 0.0f){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_CENTER,maxLblWidth); //draw Sn label
    }
    ...
    if(dat->ndat.nuclData[nuclInd].sp.val != 0.0f){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_CENTER,maxLblWidth); //draw Sp label
    }
    ...
    if(dat->ndat.nuclData[nuclInd].qalpha.val != 0.0f){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_CENTER,maxLblWidth); //draw Qalpha label
    }
    ...
    if(dat->ndat.nuclData[nuclInd].qbeta.val != 0.0f){
      ...
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 40.0f)*state->ds.uiUserScale),col,labelFontInd,255,dat->strings[dat->locStringIDs[LOCSTR_UNKNOWN]],ALIGN_CENTER,maxLblWidth); //draw Qbeta label
    }
    ...
    drawYOffsets = 1;
    if((yOffsets+drawYOffsets) <= yOffsetLimit){
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw number of known levels label      
      //SDL_Log("height: %f\n",(double)height);
      yOffsets += drawYOffsets;
    }else{
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,"(...)",ALIGN_CENTER,maxLblWidth);
    ...
    drawYOffsets = 1;
    if((yOffsets+drawYOffsets) <= yOffsetLimit){
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,tmpStr,ALIGN_CENTER,maxLblWidth); //draw number of unknown levels label      
      //SDL_Log("height: %f\n",(double)height);
      yOffsets += drawYOffsets;
    }else{
      drawTextAlignedSized(rdat,drawXPos,drawYPos+((yOffsets*labelLineSpacing + 36.0f)*state->ds.uiUserScale),col,labelFontInd,255,"(...)",ALIGN_CENTER,maxLblWidth);
...
void drawChartOfNuclides(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat){
  ...
  if(state->ds.chartZoomScale >= CHARTZOOM_LVL0){
    ...
    if(lowBoxHeight > (70.0f*state->ds.uiUserScale)){
      lowBoxHeight = 70.0f*state->ds.uiUserScale; //limit low box size
    }
  }
          ...
          if((dat->ndat.nuclData[i].Z >= (int16_t)SDL_floorf(minY))&&(dat->ndat.nuclData[i].Z >= 0)){
            if(dat->ndat.nuclData[i].Z <= (int16_t)SDL_ceilf(maxY)){
              ...
              if(state->chartView == CHARTVIEW_HALFLIFE){
                ...
                boxCol = getSnpCol(getRawValFromDB(&dat->ndat.nuclData[i].sp),dat->rules.themeRules.uiColorTheme == UITHEME_DARK);
              }else if(state->chartView == CHARTVIEW_QALPHA){
                boxCol = getQaCol(getRawValFromDB(&dat->ndat.nuclData[i].qalpha),dat->rules.themeRules.uiColorTheme == UITHEME_DARK);
              }else if(state->chartView == CHARTVIEW_QBETAMINUS){
                boxCol = getQbCol(getRawValFromDB(&dat->ndat.nuclData[i].qbeta),dat->rules.themeRules.uiColorTheme == UITHEME_DARK);
              }else if(state->chartView == CHARTVIEW_QBETAPLUS){
                boxCol = getQbCol(getRawValFromDB(&dat->ndat.nuclData[i].qbetaplus),dat->rules.themeRules.uiColorTheme == UITHEME_DARK);
              }else if(state->chartView == CHARTVIEW_QEC){
                boxCol = getQbCol(getRawValFromDB(&dat->ndat.nuclData[i].qec),dat->rules.themeRules.uiColorTheme == UITHEME_DARK);
              }else if(state->chartView == CHARTVIEW_NUMLVLS){
  ...
  if(dat->rules.themeRules.uiColorTheme == UITHEME_DARK){
    ...
    drawFlatRect(rdat,rect,blackTransparentCol);
      ...
      drawFlatRect(rdat,rect,blackCol);
    }
  }else{
    if(rdat->ssdat.takingScreenshot != 1){
    ...
    if(i >= 0.0f){
      ...
      if(i<=MAX_NEUTRON_NUM){
        ...
        if(rect.x < (state->ds.windowXRes - 1.5f*CHART_AXIS_DEPTH*state->ds.uiUserScale)){ //dodge axis label
          ...
          drawTextAlignedSized(rdat,rect.x,rect.y,axisLabelCol,FONTSIZE_NORMAL,255,tmpStr,ALIGN_CENTER,16384); //draw number label
        }
      }
    }
  }
...
void drawParentDecayQValStr(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const float yPos, const Uint8 alpha, const uint16_t nuclInd, const uint8_t decayInd, const uint32_t selMetadata){
  ...
  if(parentLvlInd < MAXNUMLVLS){
    uint16_t parentNuclInd = getParentBetaDecayNuclInd(&dat->ndat,nuclInd,decayInd);
    uint8_t dcyModeInd = getParentBetaDecayMode(&dat->ndat,nuclInd,decayInd);
    //SDL_Log("parentLvlInd: %u, parentNuclInd: %u, dcyModeInd: %u\n",parentLvlInd,parentNuclInd,dcyModeInd);
    const uint8_t mValInd = (uint8_t)((dat->ndat.levels[parentLvlInd].format >> 5U) & 7U);
    if(mValInd > 0){
      if(dat->ndat.nuclData[parentNuclInd].numIsomerMVals > 1){
        getNuclNameStr(nuclStr,&dat->ndat.nuclData[parentNuclInd],mValInd);
      }else{
        getNuclNameStr(nuclStr,&dat->ndat.nuclData[parentNuclInd],0);
      }
    }else{
      getNuclNameStr(nuclStr,&dat->ndat.nuclData[parentNuclInd],255);
    }
    getSpinParStr(jpiStr,&dat->ndat,parentLvlInd);
    getHalfLifeStr(hlStr,dat,parentLvlInd,1,1,state->ds.useLifetimes);
    if(dcyModeInd == DECAYMODE_EC){
      if(getRawValFromDB(&dat->ndat.levels[parentLvlInd].energy) > 0.0){
        valWithErr sumEVal;
        addValsFromDB(&dat->ndat.nuclData[parentNuclInd].qec,&dat->ndat.levels[parentLvlInd].energy,&sumEVal);
        getQValStr(descStr,sumEVal,1,0);
      }else{
        getQValStr(descStr,dat->ndat.nuclData[parentNuclInd].qec,1,0);
      }
      if(state->ds.useLifetimes){
        SDL_snprintf(qValStr,128,"%s (%s, %s=%s, %s=%s): %s=%s %s",dat->strings[dat->locStringIDs[LOCSTR_PARENTDECAY_LONG]],nuclStr,dat->strings[dat->locStringIDs[LOCSTR_JPI]],jpiStr,dat->strings[dat->locStringIDs[LOCSTR_LIFETIME]],hlStr,dat->strings[dat->locStringIDs[LOCSTR_QEC]],descStr,getValueUnitShortStr(dat->ndat.nuclData[parentNuclInd].qec.unit));
      }else{
      ...
      if(getRawValFromDB(&dat->ndat.levels[parentLvlInd].energy) > 0.0){
        valWithErr sumEVal;
        addValsFromDB(&dat->ndat.nuclData[parentNuclInd].qbeta,&dat->ndat.levels[parentLvlInd].energy,&sumEVal);
        getQValStr(descStr,sumEVal,1,0);
      }else{
        getQValStr(descStr,dat->ndat.nuclData[parentNuclInd].qbeta,1,0);
        
      }
      if(state->ds.useLifetimes){
        SDL_snprintf(qValStr,128,"%s (%s, %s=%s, %s=%s): %s=%s %s",dat->strings[dat->locStringIDs[LOCSTR_PARENTDECAY_LONG]],nuclStr,dat->strings[dat->locStringIDs[LOCSTR_JPI]],jpiStr,dat->strings[dat->locStringIDs[LOCSTR_LIFETIME]],hlStr,dat->strings[dat->locStringIDs[LOCSTR_QBETAMINUS]],descStr,getValueUnitShortStr(dat->ndat.nuclData[parentNuclInd].qbeta.unit));
      }else{
      ...
      if(getRawValFromDB(&dat->ndat.levels[parentLvlInd].energy) > 0.0){
        valWithErr sumEVal;
        addValsFromDB(&dat->ndat.nuclData[parentNuclInd].qalpha,&dat->ndat.levels[parentLvlInd].energy,&sumEVal);
        getQValStr(descStr,sumEVal,1,0);
      }else{
        getQValStr(descStr,dat->ndat.nuclData[parentNuclInd].qalpha,1,0);
        
      }
      if(state->ds.useLifetimes){
        SDL_snprintf(qValStr,128,"%s (%s, %s=%s, %s=%s): %s=%s %s",dat->strings[dat->locStringIDs[LOCSTR_PARENTDECAY_LONG]],nuclStr,dat->strings[dat->locStringIDs[LOCSTR_JPI]],jpiStr,dat->strings[dat->locStringIDs[LOCSTR_LIFETIME]],hlStr,dat->strings[dat->locStringIDs[LOCSTR_QALPHA]],descStr,getValueUnitShortStr(dat->ndat.nuclData[parentNuclInd].qalpha.unit));
      }else{
...
void drawNuclFullInfoBox(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const uint8_t darkTheme, const uint16_t nuclInd){
  ...
  for(uint8_t i=0; i<LLCOLUMN_ENUM_LENGTH; i++){
    allColWidth += state->ds.fullInfoColWidth[i];
  }
  allColWidth *= state->ds.uiUserScale;
  if(!(dat->ndat.nuclData[nuclInd].flags & (1U << 2))){
    //skip drawing ICC column as no data is available
    state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_ICC));
    allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_ICC]*state->ds.uiUserScale;
  }
  if(!(dat->ndat.nuclData[nuclInd].flags & (1U << 3))){
    //skip drawing mixing ratio column as no data is available
    state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_DELTA));
    allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_DELTA]*state->ds.uiUserScale;
  }
  if(allColWidth > (state->ds.windowXRes - NUCL_FULLINFOBOX_ALLCOL_DISPLAY_PADDING*state->ds.uiUserScale)){
    if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_DELTA)){
      state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_DELTA)); //don't draw mixing ratio column
      allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_DELTA]*state->ds.uiUserScale;
    }
    if(allColWidth > (state->ds.windowXRes - NUCL_FULLINFOBOX_ALLCOL_DISPLAY_PADDING*state->ds.uiUserScale)){
      if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_ICC)){
        state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_ICC)); //don't draw ICC column
        allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_ICC]*state->ds.uiUserScale;
      }
      if(allColWidth > (state->ds.windowXRes - NUCL_FULLINFOBOX_ALLCOL_DISPLAY_PADDING*state->ds.uiUserScale)){
        state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_MGAMMA)); //don't draw multipolarity column
        allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_MGAMMA]*state->ds.uiUserScale;
        if(allColWidth > (state->ds.windowXRes - NUCL_FULLINFOBOX_ALLCOL_DISPLAY_PADDING*state->ds.uiUserScale)){
          state->ds.nuclFullInfoShownColumns = (uint16_t)(state->ds.nuclFullInfoShownColumns & ~(1U << LLCOLUMN_FINALLEVEL_JPI)); //don't draw final Jpi column
          allColWidth -= state->ds.fullInfoColWidth[LLCOLUMN_FINALLEVEL_JPI]*state->ds.uiUserScale;
        }
      }
    }
  }
  ...
  const float listXPos = state->ds.fullInfoFirstColXPos - 2*UI_PADDING_SIZE*state->ds.uiUserScale;
  const float listWidth = allColWidth + 4*UI_PADDING_SIZE*state->ds.uiUserScale;
  //SDL_Log("drawYPos: %f\n",(double)drawYPos);
  ...
  for(uint32_t lvlInd = dat->ndat.nuclData[nuclInd].firstLevel; lvlInd<=(dat->ndat.nuclData[nuclInd].firstLevel+dat->ndat.nuclData[nuclInd].numLevels); lvlInd++){
    ...
    //only draw Q-values that are beyond the final level of the nuclide,
    //do not attempt to draw any additional level info
    if(lvlInd>=(dat->ndat.nuclData[nuclInd].firstLevel+dat->ndat.nuclData[nuclInd].numLevels)){
      break;
    }
    ...
    if(((drawYPos + NUCL_INFOBOX_SMALLLINE_HEIGHT*state->ds.uiUserScale*numLines) >= NUCL_FULLINFOBOX_LEVELLIST_POS_Y)&&(drawYPos <= state->ds.windowYRes)){
      ...
      //draw highlight if mouse is hovering over level
      if(rdat->ssdat.takingScreenshot != 1){
        uint8_t hoverCol = state->ds.nuclFullInfoMouseOverCol;
        uint16_t hoverLvlRow = state->ds.nuclFullInfoMouseOverLvlRow;
        uint16_t hoverLvl = state->ds.nuclFullInfoMouseOverNuclLvl;
        if(state->cms.numContextMenuItems > 0){
          hoverCol = state->ds.nuclFullInfoRightClickCol;
          hoverLvlRow = state->ds.nuclFullInfoRightClickLvlRow;
          hoverLvl = state->ds.nuclFullInfoRightClickNuclLvl;
        }
        if(hoverLvl == (lvlInd - dat->ndat.nuclData[nuclInd].firstLevel)){
          if(hoverCol < LLCOLUMN_ENUM_LENGTH){
            switch(hoverCol){
              case LLCOLUMN_ELEVEL:
              case LLCOLUMN_JPI:
              case LLCOLUMN_HALFLIFE:
                {
                  uint8_t decModeHover = 0;
                  if(hoverCol == LLCOLUMN_HALFLIFE){
                    if(dat->ndat.levels[lvlInd].numDecModes > 0){
                      if(hoverLvlRow > 0){
                        if(hoverLvlRow <= dat->ndat.levels[lvlInd].numDecModes){
                          uint16_t decayModeNucl = getDecayModeDaughterNucl(&dat->ndat,nuclInd,dat->ndat.dcyMode[dat->ndat.levels[lvlInd].firstDecMode + (uint32_t)(hoverLvlRow - 1)].type);
                          if(decayModeNucl != nuclInd){
                            decModeHover = 1;
                          }
                        }
                      }
                  ...
                  if(decModeHover){
                    //hovering over a decay mode
                    rect.w = state->ds.fullInfoColWidth[LLCOLUMN_HALFLIFE]*state->ds.uiUserScale;
                    rect.x = getFullInfoColumnXPos(state,LLCOLUMN_HALFLIFE);
                    rect.h = (NUCL_INFOBOX_SMALLLINE_HEIGHT)*state->ds.uiUserScale;
                    rect.y = drawYPos + (hoverLvlRow*NUCL_INFOBOX_SMALLLINE_HEIGHT)*state->ds.uiUserScale;
                  }else{
                    rect.w = (state->ds.fullInfoColWidth[LLCOLUMN_ELEVEL]+state->ds.fullInfoColWidth[LLCOLUMN_JPI]+state->ds.fullInfoColWidth[LLCOLUMN_HALFLIFE])*state->ds.uiUserScale;
                    rect.x = getFullInfoColumnXPos(state,LLCOLUMN_ELEVEL);
                    rect.h = (NUCL_INFOBOX_SMALLLINE_HEIGHT*numLines)*state->ds.uiUserScale;
                    rect.y = drawYPos;
                  }
              ...
              case LLCOLUMN_FINALLEVEL_JPI:
                if(hoverLvlRow < dat->ndat.levels[lvlInd].numTran){
                  rect.w = allColWidth - (getFullInfoColumnXPos(state,LLCOLUMN_EGAMMA) - state->ds.fullInfoFirstColXPos);
                  rect.x = getFullInfoColumnXPos(state,LLCOLUMN_EGAMMA);
                  rect.h = (NUCL_INFOBOX_SMALLLINE_HEIGHT)*state->ds.uiUserScale;
                  rect.y = drawYPos + (hoverLvlRow*NUCL_INFOBOX_SMALLLINE_HEIGHT)*state->ds.uiUserScale;
                  rect.x -= (NUCL_FULLINFOBOX_COL_MOUSEOVER_OFFSET*state->ds.uiUserScale);
      ...
      if(lvlInd == (dat->ndat.nuclData[nuclInd].firstLevel + dat->ndat.nuclData[nuclInd].gsLevel)){
        getLvlEnergyStr(tmpStr,&dat->ndat,lvlInd,0); //ground state energy has no error, is just 0.0
      }else{
      ...
      if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
        ...
        if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.levels[lvlInd].hasComment & (uint8_t)(1U << LCOMMENT_ELEVEL))){
          drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata);
        }else{
          drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata);
        }
      }else{
        if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.levels[lvlInd].hasComment & (uint8_t)(1U << LCOMMENT_ELEVEL))){
          drawTextAlignedSized(rdat,drawXPos,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384);
        }else{
          drawTextAlignedSized(rdat,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384);
        }
      }
      ...
      if(mValInd > 0){
        ...
        if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
          ...
          if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
          drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,mValStr,ALIGN_LEFT,16384,strMetadata);
        }else{
          drawTextAlignedSized(rdat,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,mValStr,ALIGN_LEFT,16384);
        }
        lvlInfoRow++;
      }
      if(slInd > 0){
        ...
        if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
          ...
          strMetadata |= (uint32_t)(lvlInfoRow << 24); //bits 24-31 give row
          if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
          drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,slStr,ALIGN_LEFT,16384,strMetadata);
        }else{
          drawTextAlignedSized(rdat,drawXPos,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,slStr,ALIGN_LEFT,16384);
        }
      }
        ...
        for(int8_t i=0; i<dat->ndat.levels[lvlInd].numDecModes; i++){
          ...
          if((drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f))&&(decayModeNucl < dat->ndat.numNucl)&&(decayModeNucl != nuclInd)){
            ...
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.levels[lvlInd].hasComment & (uint8_t)(1U << LCOMMENT_DECAYMODE))){
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable decay mode label
            }else{
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable decay mode label
            }
            ...
            if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.levels[lvlInd].hasComment & (uint8_t)(1U << LCOMMENT_DECAYMODE))){
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw decay mode label
            }else{
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw decay mode label
            }
          }else{
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.levels[lvlInd].hasComment & (uint8_t)(1U << LCOMMENT_DECAYMODE))){
              drawTextAlignedSized(rdat,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw decay mode label
            }else{
              drawTextAlignedSized(rdat,drawXPos+(2*UI_PADDING_SIZE*state->ds.uiUserScale),drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw decay mode label
            }
          }
        }
      }
        ...
        for(uint16_t i=0; i<dat->ndat.levels[lvlInd].numTran; i++){
          ...
          if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
            ...
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_EGAMMA))){
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition energy label
            }else{
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition energy label
            }
          }else{
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_EGAMMA))){
              drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition energy label
            }else{
              drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition energy label
            }
          }
          ...
          if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
            ...
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_IGAMMA))){
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition intensity label
            }else{
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition intensity label
            }
          }else{
            if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_IGAMMA))){
              drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition intensity label
            }else{
              drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition intensity label
            }
          }
          ...
          if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_MGAMMA)){
            ...
            if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
              ...
              if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_MGAMMA))){
                drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition multipolarity label
              }else{
                drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition multipolarity label
              }
            }else{
              if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_MGAMMA))){
                drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition multipolarity label
              }else{
                drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition multipolarity label
              }
            }
          ...
          if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_DELTA)){
            ...
            if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
              ...
              if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_DELTA))){
                drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition mixing ratio label
              }else{
                drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition mixing ratio label
              }
            }else{
              if((state->ds.useLevelListCommentTooltips == 1)&&(dat->ndat.tran[dat->ndat.levels[lvlInd].firstTran + i].hasComment & (uint8_t)(1U << TCOMMENT_DELTA))){
                drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtColBlue,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition mixing ratio label
              }else{
                drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition mixing ratio label
              }
            }
          ...
          if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_ICC)){
            ...
            if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
              ...
              strMetadata |= (uint32_t)((uint32_t)(i) << 24); //bits 24-31 give row
              if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable transition ICC label
            }else{
              drawTextAlignedSized(rdat,drawXPosTran,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw transition ICC label
            }
          ...
          if(dat->ndat.tran[(uint32_t)(dat->ndat.levels[lvlInd].firstTran + i)].finalLvlOffset != 0){
            ...
            if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
              ...
              strMetadata |= (uint32_t)((uint32_t)(i) << 24); //bits 24-31 give row
              if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
              drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosFL,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384,strMetadata); //draw selectable final level energy label
            }else{
              drawTextAlignedSized(rdat,drawXPosFL,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_LEFT,16384); //draw final level energy label
            }
            if(state->ds.nuclFullInfoShownColumns & (1U << LLCOLUMN_FINALLEVEL_JPI)){
              ...
              if(drawYPos >= (NUCL_FULLINFOBOX_LEVELLIST_POS_Y*state->ds.uiUserScale - 1.0f)){
                ...
                strMetadata |= (uint32_t)((uint32_t)(i) << 24); //bits 24-31 give row
                if((state->tss.selStrsModifiable)&&(strMetadata == state->ds.nuclFullInfoSelStrMetadata)){state->tss.selectedStr = state->tss.numSelStrs;}
                drawSelectableTextAlignedSizedWithMetadata(rdat,&state->tss,drawXPosFL,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_RIGHT,16384,strMetadata); //draw selectable final level spin-parity label
              }else{
                drawTextAlignedSized(rdat,drawXPosFL,drawYPos,lvlTxtCol,lvlFontInd,txtAlpha,tmpStr,ALIGN_RIGHT,16384); //draw final level spin-parity label
              }
            }
          }
  ...
  if(rdat->ssdat.takingScreenshot != 1){
    ...
    //search button
    drawIconButton(&dat->rules.themeRules,rdat,(int16_t)(state->ds.uiElemPosX[UIELEM_SEARCH_BUTTON]),(int16_t)(state->ds.uiElemPosY[UIELEM_SEARCH_BUTTON] - txtYOffset),state->ds.uiElemWidth[UIELEM_SEARCH_BUTTON],getHighlightState(state,UIELEM_SEARCH_BUTTON),1.0f,UIICON_SEARCH);
  }
...
void drawAboutBox(const app_data *restrict dat, const app_state *restrict state, resource_data *restrict rdat){
  ...
  drawTextAlignedSized(rdat,aboutBoxPanelRect.x+(2*UI_TILE_SIZE + 2*UI_PADDING_SIZE)*state->ds.uiUserScale,aboutBoxPanelRect.y+ABOUT_BOX_VERSION_Y*state->ds.uiUserScale,textCol,FONTSIZE_SMALL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_ABOUTSTR_VERSION]],ALIGN_LEFT,16384);
  drawTextAlignedSized(rdat,aboutBoxPanelRect.x+(aboutBoxPanelRect.w/2),aboutBoxPanelRect.y+ABOUT_BOX_STR1_Y*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL_BOLD,alpha8,dat->strings[dat->locStringIDs[LOCSTR_ABOUTSTR_1]],ALIGN_CENTER,(Uint16)(aboutBoxPanelRect.w - 16*UI_PADDING_SIZE*state->ds.uiUserScale));
  drawTextAlignedSized(rdat,aboutBoxPanelRect.x+(aboutBoxPanelRect.w/2),aboutBoxPanelRect.y+ABOUT_BOX_STR2_Y*state->ds.uiUserScale,textCol,FONTSIZE_SMALL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_ABOUTSTR_2]],ALIGN_CENTER,(Uint16)(aboutBoxPanelRect.w - 16*UI_PADDING_SIZE*state->ds.uiUserScale));
  drawTextAlignedSized(rdat,aboutBoxPanelRect.x+(aboutBoxPanelRect.w/2),aboutBoxPanelRect.y+ABOUT_BOX_STR3_Y*state->ds.uiUserScale,textCol,FONTSIZE_SMALL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_ABOUTSTR_3]],ALIGN_CENTER,(Uint16)(aboutBoxPanelRect.w - 16*UI_PADDING_SIZE*state->ds.uiUserScale));
  drawTextAlignedSized(rdat,aboutBoxPanelRect.x+(aboutBoxPanelRect.w/2),aboutBoxPanelRect.y+ABOUT_BOX_STR4_Y*state->ds.uiUserScale,textCol,FONTSIZE_SMALL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_ABOUTSTR_4]],ALIGN_CENTER,(Uint16)(aboutBoxPanelRect.w - 16*UI_PADDING_SIZE*state->ds.uiUserScale));
  //close button
...
uint16_t getStrIndForReactionMode(const app_data *restrict dat, const drawing_state *restrict ds){
  switch(ds->reactionModeInd){
    case REACTIONMODE_HIGHLIGHT:
      return dat->locStringIDs[LOCSTR_PREF_REACTIONMODE_HIGHLIGHT];
      break;
    case REACTIONMODE_EXCLUDE:
    default:
      return dat->locStringIDs[LOCSTR_PREF_REACTIONMODE_EXCLUDE];
      break;
  }
}
...
void drawPrefsDialog(const app_data *restrict dat, const app_state *restrict state, resource_data *restrict rdat){
  ...
  drawTextAlignedSized(rdat,prefsDialogPanelRect.x+(PREFS_DIALOG_PREFCOL1_X+UI_TILE_SIZE+2*UI_PADDING_SIZE)*state->ds.uiUserScale,prefsDialogPanelRect.y+(PREFS_DIALOG_PREFCOL1_Y+6.0f+(2*PREFS_DIALOG_PREF_Y_SPACING)+(2*UI_PADDING_SIZE))*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_PREF_SHELLCLOSURE]],ALIGN_LEFT,(Uint16)(prefsDialogPanelRect.w));
  drawTextAlignedSized(rdat,prefsDialogPanelRect.x+(PREFS_DIALOG_PREFCOL1_X+UI_TILE_SIZE+2*UI_PADDING_SIZE)*state->ds.uiUserScale,prefsDialogPanelRect.y+(PREFS_DIALOG_PREFCOL1_Y+6.0f+(3*PREFS_DIALOG_PREF_Y_SPACING)+(2*UI_PADDING_SIZE))*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_PREF_LIFETIME]],ALIGN_LEFT,(Uint16)(prefsDialogPanelRect.w));
  drawTextAlignedSized(rdat,prefsDialogPanelRect.x+(PREFS_DIALOG_PREFCOL1_X+UI_PADDING_SIZE)*state->ds.uiUserScale,prefsDialogPanelRect.y+(PREFS_DIALOG_PREFCOL1_Y+6.0f+(4.1f*PREFS_DIALOG_PREF_Y_SPACING)+(2*UI_PADDING_SIZE))*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_PREF_LEVELLIST_HEADER]],ALIGN_LEFT,(Uint16)(prefsDialogPanelRect.w));
  drawTextAlignedSized(rdat,prefsDialogPanelRect.x+(PREFS_DIALOG_PREFCOL1_X+UI_TILE_SIZE+6*UI_PADDING_SIZE)*state->ds.uiUserScale,prefsDialogPanelRect.y+(PREFS_DIALOG_PREFCOL1_Y+6.0f+(5.15f*PREFS_DIALOG_PREF_Y_SPACING)+(2*UI_PADDING_SIZE))*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_PREF_LEVELLIST_SEPARATION]],ALIGN_LEFT,(Uint16)(prefsDialogPanelRect.w));
  drawTextAlignedSized(rdat,prefsDialogPanelRect.x+(PREFS_DIALOG_PREFCOL1_X+UI_TILE_SIZE+6*UI_PADDING_SIZE)*state->ds.uiUserScale,prefsDialogPanelRect.y+(PREFS_DIALOG_PREFCOL1_Y+6.0f+(6.15f*PREFS_DIALOG_PREF_Y_SPACING)+(2*UI_PADDING_SIZE))*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,alpha8,dat->strings[dat->locStringIDs[LOCSTR_PREF_LEVELLIST_THRESHOLD]],ALIGN_LEFT,(Uint16)(prefsDialogPanelRect.w));
...
void drawReactionModeMenu(const app_data *restrict dat, const app_state *restrict state, resource_data *restrict rdat){
  
  float alpha = 1.0f;
  float yOffset = 0;
  if(state->ds.uiAnimPlaying & (1U << UIANIM_REACTIONMODE_MENU_HIDE)){
    alpha = (float)(1.0f*juice_smoothStart2(state->ds.timeLeftInUIAnimation[UIANIM_REACTIONMODE_MENU_HIDE]/(UI_ANIM_LENGTH)));
  }else if(state->ds.uiAnimPlaying & (1U << UIANIM_REACTIONMODE_MENU_SHOW)){
    alpha = (float)(1.0f*juice_smoothStop2(1.0f - state->ds.timeLeftInUIAnimation[UIANIM_REACTIONMODE_MENU_SHOW]/UI_ANIM_LENGTH));
    yOffset = (-30.0f*state->ds.uiUserScale*juice_smoothStart2(state->ds.timeLeftInUIAnimation[UIANIM_REACTIONMODE_MENU_SHOW]/(UI_ANIM_LENGTH)));
  }
  //SDL_Log("alpha: %f\n",(double)alpha);
  
  //draw menu background
  SDL_FRect drawRect;
  drawRect.x = state->ds.uiElemPosX[UIELEM_PREFS_REACTIONMODE_MENU];
  drawRect.y = ((float)state->ds.uiElemPosY[UIELEM_PREFS_REACTIONMODE_MENU] + yOffset);
  ...
  const int16_t arrowX = (int16_t)(state->ds.uiElemPosX[UIELEM_PREFS_DIALOG_REACTIONMODE_DROPDOWN] + 0.45f*state->ds.uiElemWidth[UIELEM_PREFS_DIALOG_REACTIONMODE_DROPDOWN]);
  drawMenuBGWithArrow(&dat->rules.themeRules,rdat,drawRect,arrowX,alpha);
  
  //draw menu item highlight
  if(state->ds.timeLeftInUIAnimation[UIANIM_REACTIONMODE_MENU_HIDE]==0.0f){
    for(uint8_t i=1;i<=UISCALE_ENUM_LENGTH;i++){
      drawRect.x = state->ds.uiElemPosX[UIELEM_PREFS_REACTIONMODE_MENU-i];
      drawRect.y = (state->ds.uiElemPosY[UIELEM_PREFS_REACTIONMODE_MENU-i] + yOffset);
      drawRect.w = state->ds.uiElemWidth[UIELEM_PREFS_REACTIONMODE_MENU-i];
...
void drawContextMenu(const app_data *restrict dat, const app_state *restrict state, resource_data *restrict rdat){
  ...
  if(state->cms.useHeaderText){
    ...
    drawTextAlignedSized(rdat,drawRect.x,drawRect.y,textCol,FONTSIZE_LARGE_BOLD,txtAlpha,state->cms.headerText,ALIGN_LEFT,16384);
  }
  ...
  for(uint8_t i=0;i<state->cms.numContextMenuItems;i++){
    switch(state->cms.contextMenuItems[i]){
      case CONTEXTITEM_COPY:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CONTEXT_COPYTEXT]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_NUCLNAME:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CONTEXT_COPY_NUCLNAME]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_NUCLINFO:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CONTEXT_COPY_NUCLINFO]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_CHART_PROPERTY:
        switch(state->chartView){
          case CHARTVIEW_HALFLIFE:
            ...
            drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,tmpStr,ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
            break;
      ...
      case CONTEXTITEM_GOTO_LEVEL:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CLICKACTION_GOTOLEVEL]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_GOTO_DAUGHTER:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CLICKACTION_GOTODAUGHTER]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_SHOW_COINC:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CLICKACTION_SHOWCOINC]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_SHOW_SAME_JPI:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CLICKACTION_SHOWSAMEJPI]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      case CONTEXTITEM_COPY_COMMENT:
        drawTextAlignedSized(rdat,drawRect.x,drawRect.y + (0.4f + (float)i)*CONTEXT_MENU_ITEM_SPACING*state->ds.uiUserScale,textCol,FONTSIZE_NORMAL,txtAlpha,dat->strings[dat->locStringIDs[LOCSTR_CONTEXT_COPY_COMMENT]],ALIGN_LEFT,(Uint16)(drawRect.w - (PANEL_EDGE_SIZE + 6*UI_PADDING_SIZE)*state->ds.uiUserScale));
        break;
      default:
        break;
    }
...
void drawUI(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat){
  ...
  if(bp_check128(&state->ds.shownElements,UIELEM_CHARTOFNUCLIDES)){
    ...
    if(rdat->ssdat.takingScreenshot != 1){
      ...
      drawIcon(&dat->rules.themeRules,rdat,(int16_t)(state->ds.uiElemPosX[UIELEM_RECENTER_BUTTON] - yOffset),state->ds.uiElemPosY[UIELEM_RECENTER_BUTTON],state->ds.uiElemWidth[UIELEM_RECENTER_BUTTON],getHighlightState(state,UIELEM_RECENTER_BUTTON),1.0f,UIICON_RECENTER);
    }
  }
  ...
  if(bp_check128(&state->ds.shownElements,UIELEM_NUCL_INFOBOX)){
    ...
    drawNuclFullInfoBox(dat,state,rdat,dat->rules.themeRules.uiColorTheme == UITHEME_DARK,state->chartSelectedNucl);
  ...
  if(rdat->ssdat.takingScreenshot != 1){
    ...
    if(state->ds.uiAnimPlaying & (1U << UIANIM_CHART_FADEIN)){
      ...
      drawFlatBG(&state->ds,rdat,overlayCol);
    }
  }
