		...
		if(filelist){
			...
			while(*filelist){
				...
				if(strncmp(*filelist,"file://",7)==0){
					SDL_strlcpy(fileName,(*filelist)+7,255);
				}else{
					SDL_strlcpy(fileName,(*filelist),255);
				}
				//SDL_Log("Filename: %s\n",fileName);
				//check for file extension
				const char *dot = SDL_strrchr(fileName,'.');
				if((dot==NULL)||(SDL_strcmp(dot,".png")!=0)){
					SDL_strlcat(fileName,".png",255);
				}
				//SDL_Log("Filename: %s\n",fileName);
  ...
  state->ds.uiAnimPlaying |= (1U << uiAnim);
	...
	switch(uiAnim){
		...
		case UIANIM_NUCLINFOBOX_HIDE:
			changeUIState(dat,state,rdat,UISTATE_CHARTONLY); //make info box un-interactable
			break;
		default:
			break;
	}
  ...
  for(uint8_t i=0;i<UIANIM_ENUM_LENGTH;i++){
    if(state->ds.uiAnimPlaying & (uint32_t)(1U << i)){
      ...
      if((state->ds.timeLeftInUIAnimation[i] <= 0.0f)||(state->ds.useUIAnimations == 0)){
        ...
        stopUIAnimation(dat,state,rdat,i);
      }
    }
  }
}
...
void updateDrawingState(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const float deltaTime){
	if(state->ds.zoomFinished){
		...
		changeUIState(dat,state,rdat,state->uiState); //update interactable elements (specifically zoom buttons)
	}
...
// gets the number of levels in the nuclide which have a beta or EC decay mode
uint16_t getNumBetaDecayingLvls(const ndata *restrict nd, const uint16_t nuclInd){
	uint16_t numLvls = 0;
		...
		if(!ambiguousLvl){
			for(int8_t j=0; j<nd->levels[i].numDecModes; j++){
				...
				if((decUnitType != VALUETYPE_LESSTHAN)&&(decUnitType != VALUETYPE_LESSOREQUALTHAN)&&(decUnitType != VALUETYPE_UNKNOWN)){
					if((nd->dcyMode[dcyModeInd].type == DECAYMODE_ECANDBETAPLUS)||(nd->dcyMode[dcyModeInd].type == DECAYMODE_BETAMINUS)||
					(nd->dcyMode[dcyModeInd].type == DECAYMODE_BETAPLUS)||(nd->dcyMode[dcyModeInd].type == DECAYMODE_EC)){
						numLvls++;
...
void changeUIState(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const uint8_t newState){
  ...
  switch(state->uiState){
		...
		case UISTATE_FULLLEVELINFOWITHMENU:
			...
			updateSingleUIElemPosition(dat,state,rdat,UIELEM_SEARCH_RESULT);
		...
		case UISTATE_CHARTWITHMENU:
			...
			if((bp_check128(&state->ds.shownElements,UIELEM_CHARTVIEW_MENU))&&(state->ds.timeLeftInUIAnimation[UIANIM_CHARTVIEW_MENU_HIDE]==0.0f)){
				...
				bp_set128(&state->interactableElement,UIELEM_CVM_NUMBETADCY);
...
void setInfoBoxDimensions(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const uint16_t selNucl){
  ...
  if(useIsomer){
	...
	if(state->ds.infoBoxPrevX >= 0.0f){ //initialized to -1 on program start
		...
		startUIAnimation(dat,state,rdat,UIANIM_NUCLINFOBOX_MORPH);
...
void setSelectedNuclOnChart(const app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const uint16_t N, const uint16_t Z, const uint8_t forcePan){
	...
	if((selNucl < MAXNUMNUCL)&&(selNucl != state->chartSelectedNucl)){
		...
		startUIAnimation(dat,state,rdat,UIANIM_NUCLHIGHLIGHT_SHOW);
		if(forcePan==1){
			...
			{//prevent -Wjump-misses-init
				...
				switch(state->chartView){
					...
					case CHARTVIEW_NUMBETADCY:
						{//prevent -Wjump-misses-init
							const uint16_t numLvls = getNumBetaDecayingLvls(&dat->ndat,state->cms.selectionInd);
							SDL_snprintf(state->copiedTxt,MAX_SELECTABLE_STR_LEN,"%s: %u",dat->strings[dat->locStringIDs[LOCSTR_CHARTVIEW_NUMBETADCY]],numLvls);
							SDL_SetClipboardText(state->copiedTxt);
							//SDL_Log("Copied text to clipboard: %s\n",SDL_GetClipboardText());
						}
						break;
...
void uiElemClickAction(app_data *restrict dat, app_state *restrict state, resource_data *restrict rdat, const uint8_t doubleClick, const uint8_t uiElemID){
  ...
  switch(uiElemID){
    case UIELEM_MENU_BUTTON:
      ...
      changeUIState(dat,state,rdat,state->lastUIState); //restore previous interactable elements
      startUIAnimation(dat,state,rdat,UIANIM_MODAL_BOX_HIDE); //about box will be closed after animation finishes
      break;
      ...
      break;
		...
		case UIELEM_NUCL_INFOBOX_ALLLEVELSBUTTON:
			...
			if(state->chartSelectedNucl < MAXNUMNUCL){
				...
				state->ds.selectedRxn = 0;
				state->lastOpenedMenu = UIELEM_PRIMARY_MENU;
				startUIAnimation(dat,state,rdat,UIANIM_NUCLINFOBOX_EXPAND);
				setFullLevelInfoDimensions(dat,state,rdat,state->chartSelectedNucl);
			}
			break;
		case UIELEM_NUCL_FULLINFOBOX_BACKBUTTON:
			startUIAnimation(dat,state,rdat,UIANIM_NUCLINFOBOX_TXTFADEOUT); //menu will be closed after animation finishes
			break;
		case UIELEM_NUCL_FULLINFOBOX_SCROLLBAR:
      ...
      state->clickedUIElem = UIELEM_ENUM_LENGTH; //'unclick' the menu button
		...
		case UIELEM_CVM_NUMISOMERS_1MIN:
			...
			changeUIState(dat,state,rdat,UISTATE_CHARTONLY); //prevents mouseover from still highlighting buttons while the menu closes
			break;
		case UIELEM_CVM_NUMBETADCY:
			startUIAnimation(dat,state,rdat,UIANIM_CHARTVIEW_MENU_HIDE); //menu will be closed after animation finishes
			state->clickedUIElem = UIELEM_ENUM_LENGTH; //'unclick' the menu button
			state->chartView = CHARTVIEW_NUMBETADCY;
			changeUIState(dat,state,rdat,UISTATE_CHARTONLY); //prevents mouseover from still highlighting buttons while the menu closes
			break;
