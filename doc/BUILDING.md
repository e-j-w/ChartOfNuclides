- [Linux or MacOS (manual build)](#linux-or-macos-manual-build)
- [Linux (Flatpak)](#linux-flatpak)
- [Windows](#windows)

# Linux or MacOS (manual build)
...
Some older Linux distros may not package SDL3 and its libraries since they are fairly recent, in that case you will have manually compile them.

**In Arch Linux:**

```
pacman -Syu base-devel sdl3 sdl3_image sdl3_ttf pkgconf
```
...
Two executables will be built: `proc_data` (which generates the data package containing the nuclear structure database used by the main application), and `chart` (the main application).

[Build the data file](#build-data-file), which is neccessary to run the application. Once it is built, the application can be run:
